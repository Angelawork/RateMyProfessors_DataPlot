...
class HashTableTests {
  ...
  void getEntriesTest1() {
    ...
    assertEquals(entries.stream().map(MyPair::getKey).sorted().collect(Collectors.toList()), expectedKeys);
    assertEquals(entries.stream().map(MyPair::getValue).sorted().collect(Collectors.toList()), expectedVals);
  }
...
class DataAnalyzerTests {
  Parser p = new Parser("/src/RateMyProf_Data_Gendered_Sample.csv");
